...
class GN():
    ...
    def copyLines(self, seq, head, gt_seq, tail=-1, tag=0):
        ...
        '''
        if tt_tag:
            basic_dir = mot_dataset_dir + \
                'MOT%d/test/MOT%d-%02d-%s/' % (year, year, seq, type)
        else:
            basic_dir = mot_dataset_dir + \
                'MOT%d/train/MOT%d-%02d-%s/' % (year, year, seq, type)
    ...
    def loadModel(self):
        tail = 13
        self.Uphi = torch.load(model_dir + 'uphi_%d.pth' %
                               tail).to(self.device)
        self.Ephi = torch.load(model_dir + 'ephi_%d.pth' %
                               tail).to(self.device)
    ...
    def linearModel(self, out, attr1, attr2):
        ...
        if t > f_gap:
            return
        frame = int(attr1[0])
        x1, y1, w1, h1 = float(attr1[2]), float(
            attr1[3]), float(attr1[4]), float(attr1[5])
        x2, y2, w2, h2 = float(attr2[2]), float(
            attr2[3]), float(attr2[4]), float(attr2[5])
    ...
    def evaluation(self, head, tail, gtFile, outFile):
        ...
        while step < tail:
            ...
            for edge in self.train_set.candidates:
                ...
                v2 = self.train_set.getMotion(
                    0, vr_index, vs_index, line_con[self.cur][vs_index][-1])
            ...
            for (i, j) in results:
                ...
                if ret[i][j] >= tau_threshold:
                    continue
                look_up.remove(j)
                self.train_set.updateVelocity(
                    i, j, line_con[self.cur][i][-1], False)
    ...
    try:
        ...
        for t in types:
            ...
            for i in range(len(seqs)):
                ...
                print('The sequence:', seq_index,
                      '- The length of the training data:', seq_len)
                ...
                print('     Recover the number missing detections:',
                      gn.missingCounter)
